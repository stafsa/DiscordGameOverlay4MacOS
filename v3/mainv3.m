...
static NSString * const DO_CLIENT_ID     = @"<insert here yours ig>";
static NSString * const DO_CLIENT_SECRET = @"<insert here yours ig>";
static NSString * const DO_REDIRECT_URI  = @"https://localhost";
static const BOOL DO_DEFAULT_DEBUG_MODE = NO;
...
static NSString *DOOAuthSessionDirectory(void) {
    static NSString *directory = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *preferred = [[NSHomeDirectory() stringByAppendingPathComponent:@"Library/Application Support"]
                               stringByAppendingPathComponent:@"com.stafsa.dofm"];
        NSError *error = nil;
        BOOL created = [[NSFileManager defaultManager] createDirectoryAtPath:preferred
                                                  withIntermediateDirectories:YES
                                                                   attributes:@{ NSFilePosixPermissions: @0700 }
                                                                        error:&error];
        BOOL isDirectory = NO;
        BOOL usable = created && [[NSFileManager defaultManager] fileExistsAtPath:preferred isDirectory:&isDirectory] && isDirectory &&
            [[NSFileManager defaultManager] isWritableFileAtPath:preferred];
        if (usable) {
            [[NSFileManager defaultManager] setAttributes:@{ NSFilePosixPermissions: @0700 }
                                             ofItemAtPath:preferred
                                                    error:nil];
            directory = preferred;
        } else {
            directory = DOOverlayDataDirectory();
            DOLog(@"OAuth session storage fallback to host data directory%@", error ? [NSString stringWithFormat:@" (%@)", error.localizedDescription] : @"");
        }
    });
    return directory;
}

static NSString *DOOAuthSessionFilePath(void) {
    return [DOOAuthSessionDirectory() stringByAppendingPathComponent:@"oauth_session.json"];
}

static NSDictionary *DOOAuthSessionFromFile(void) {
    NSData *data = [NSData dataWithContentsOfFile:DOOAuthSessionFilePath()];
...
static BOOL DOSaveOAuthSession(NSDictionary *session) {
    NSString *accessToken = [session[@"access_token"] isKindOfClass:[NSString class]] ? session[@"access_token"] : nil;
    if (accessToken.length == 0) return NO;
    NSData *data = [NSJSONSerialization dataWithJSONObject:session options:0 error:nil];
    if (data.length == 0) return NO;
    NSString *path = DOOAuthSessionFilePath();
    BOOL saved = [data writeToFile:path options:NSDataWritingAtomic error:nil];
    if (saved) {
...
- (BOOL)sendHandshake {
    ...
    NSData *packet = DORPCPacket(0, handshake);
    BOOL ok = [self writeData:packet];
    DOLog(@"IPC handshake sent=%d bytes=%lu", ok, (unsigned long)packet.length);
    return ok;
}
...
- (NSData *)readExact:(NSUInteger)length {
    ...
    while (received < length) {
        ...
        if (n <= 0) {
            if (n == 0) {
                DOLog(@"IPC peer closed requested=%lu received=%lu",
                      (unsigned long)length,
                      (unsigned long)received);
            } else {
                DOLog(@"IPC read failed requested=%lu received=%lu errno=%d",
                      (unsigned long)length,
                      (unsigned long)received,
                      errno);
            }
...
- (NSDictionary *)requestOAuthTokenSyncWithBody:(NSString *)body {
    ...
    __block NSDictionary *tokenResponse = nil;
    ...
    [[NSURLSession sharedSession] dataTaskWithRequest:req
        ...
        if (!error && data.length > 0) {
            ...
            if ([json isKindOfClass:[NSDictionary class]]) tokenResponse = json;
        }
        dispatch_semaphore_signal(sem);
    }];

    [task resume];
    long wait = dispatch_semaphore_wait(sem, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(16 * NSEC_PER_SEC)));
    NSString *accessToken = [tokenResponse[@"access_token"] isKindOfClass:[NSString class]] ? tokenResponse[@"access_token"] : nil;
    if (wait != 0 || accessToken.length == 0) {
        DOLog(@"OAuth token request failed status=%ld timedOut=%d", (long)status, wait != 0);
        return nil;
    }
    return tokenResponse;
}

- (NSDictionary *)exchangeTokenSessionSync:(NSString *)code {
    ...
    return [self requestOAuthTokenSyncWithBody:body];
}

- (NSDictionary *)refreshTokenSessionSync:(NSString *)refreshToken {
    if (refreshToken.length == 0) return nil;
    NSString *body = [NSString stringWithFormat:@"grant_type=refresh_token&refresh_token=%@",
                      [self urlEncodedString:refreshToken]];
    return [self requestOAuthTokenSyncWithBody:body];
}

- (NSDictionary *)sessionByAddingExpiryToTokenResponse:(NSDictionary *)response previous:(NSDictionary *)previous {
    NSString *accessToken = [response[@"access_token"] isKindOfClass:[NSString class]] ? response[@"access_token"] : nil;
    if (accessToken.length == 0) return nil;
    NSString *refreshToken = [response[@"refresh_token"] isKindOfClass:[NSString class]] ? response[@"refresh_token"] : previous[@"refresh_token"];
    NSTimeInterval expiresIn = [response[@"expires_in"] respondsToSelector:@selector(doubleValue)] ? [response[@"expires_in"] doubleValue] : 0;
    NSMutableDictionary *session = [@{
        @"access_token": accessToken,
        @"expires_at": @([[NSDate date] timeIntervalSince1970] + MAX(0, expiresIn))
    } mutableCopy];
    if (refreshToken.length > 0) session[@"refresh_token"] = refreshToken;
    return session;
}

- (BOOL)authenticateSavedSessionIfPossible {
    NSDictionary *session = DOOAuthSessionFromFile();
    if (!session) return NO;
    NSTimeInterval expiresAt = [session[@"expires_at"] respondsToSelector:@selector(doubleValue)] ? [session[@"expires_at"] doubleValue] : 0;
    NSString *accessToken = [session[@"access_token"] isKindOfClass:[NSString class]] ? session[@"access_token"] : nil;
    if (accessToken.length > 0 && expiresAt > [[NSDate date] timeIntervalSince1970] + 60.0) {
        self.accessToken = accessToken;
        DOLog(@"OAuth session restored from disk");
    ...
    NSDictionary *response = [self refreshTokenSessionSync:refreshToken];
    NSDictionary *refreshed = [self sessionByAddingExpiryToTokenResponse:response previous:session];
    if (!refreshed || !DOSaveOAuthSession(refreshed)) return NO;
    self.accessToken = refreshed[@"access_token"];
    DOLog(@"OAuth session refreshed from disk");
    [self authenticate:self.accessToken];
    return YES;
}
...
- (NSMutableDictionary *)normalizedVoiceStateEntryFrom:(NSDictionary *)vs {
    ...
    BOOL streaming = [voiceState[@"self_stream"] boolValue] || [voiceState[@"stream"] boolValue] ||
        [vs[@"self_stream"] boolValue] || [vs[@"stream"] boolValue];
...
- (void)upsertVoiceState:(NSDictionary *)entry {
    ...
    if (idx != NSNotFound) {
        ...
        NSDictionary *incomingVoiceState = [entry[@"voice_state"] isKindOfClass:[NSDictionary class]] ? entry[@"voice_state"] : @{};
        BOOL incomingHasStreaming = incomingVoiceState[@"self_stream"] != nil || incomingVoiceState[@"stream"] != nil ||
            entry[@"self_stream"] != nil || entry[@"stream"] != nil;
        if (!incomingHasStreaming && [prev[@"voice_state"][@"self_stream"] respondsToSelector:@selector(boolValue)]) {
            normalized[@"voice_state"][@"self_stream"] = prev[@"voice_state"][@"self_stream"];
        }
        arr[idx] = normalized;
    } else {
...
- (void)publishSharedState {
    ...
    for (NSDictionary *vs in states) {
        ...
        u.streaming = [voiceState[@"self_stream"] boolValue] || [voiceState[@"stream"] boolValue];
        if (u.streaming) DOLog(@"voice stream detected user=%@ name=%@", u.userId, u.name);
...
- (void)processMessage:(NSDictionary *)msg {
    ...
    if ([evt isEqualToString:@"READY"]) {
        ...
        self.didAuthenticate = NO;
        self.didAuthorize = NO;
        if (![self authenticateSavedSessionIfPossible]) [self authorize];
        return;
    }

    if ([evt isEqualToString:@"ERROR"]) {
        NSLog(@"[ipc] RPC ERROR: %@", msg[@"data"]);
        DOLog(@"IPC ERROR cmd=%@ nonce=%@ data=%@", cmd ?: @"", msg[@"nonce"] ?: @"", msg[@"data"]);
    ...
    if ([cmd isEqualToString:@"AUTHORIZE"] && [msg[@"data"] isKindOfClass:[NSDictionary class]]) {
        ...
        if (code.length > 0) {
            NSDictionary *response = [self exchangeTokenSessionSync:code];
            NSDictionary *session = [self sessionByAddingExpiryToTokenResponse:response previous:nil];
            BOOL saved = session ? DOSaveOAuthSession(session) : NO;
            DOLog(@"token exchange: %s sessionSaved=%d", (session ? "ok" : "failed"), saved);
            if (session) {
...
- (void)readLoop {
    while (1) {
        ...
        if (!header || header.length != 8) {
            DOLog(@"IPC read loop ended before frame header");
            break;
        }
        ...
        if (len <= 0 || len > 10 * 1024 * 1024) {
            DOLog(@"IPC invalid frame header op=%d length=%d", op, len);
            break;
        }

        NSData *payload = [self readExact:(NSUInteger)len];
        if (!payload || payload.length != (NSUInteger)len) {
            DOLog(@"IPC read loop ended while reading frame op=%d length=%d", op, len);
            break;
        }

        if (op != 1) {
            NSString *body = [[NSString alloc] initWithData:payload encoding:NSUTF8StringEncoding] ?: @"<non-UTF8>";
            DOLog(@"IPC non-frame opcode=%d length=%d body=%@", op, len, body);
            continue;
        }
        ...
        if ([json isKindOfClass:[NSDictionary class]]) {
            ...
            DOLog(@"IPC received invalid JSON frame length=%d", len);
        }
    }
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    for (DOUser *user in self.users) {
        ...
        if (user.streaming) {
            NSRect liveBadge = NSMakeRect(nameEndX + 7.0, NSMidY(avatarRect) - 9.0, 42.0, 18.0);
            [NSGraphicsContext saveGraphicsState];
            if (dimUser) CGContextSetAlpha(NSGraphicsContext.currentContext.CGContext, 0.50);
            NSBezierPath *livePath = [NSBezierPath bezierPathWithRoundedRect:liveBadge xRadius:9.0 yRadius:9.0];
            [[NSColor colorWithCalibratedRed:0.93 green:0.15 blue:0.19 alpha:1.0] setFill];
            [livePath fill];
            NSDictionary *liveAttrs = @{
                NSFontAttributeName: [NSFont systemFontOfSize:10.0 weight:NSFontWeightBold],
                NSForegroundColorAttributeName: NSColor.whiteColor
            };
            NSString *liveText = @"LIVE";
            NSSize liveTextSize = [liveText sizeWithAttributes:liveAttrs];
            [liveText drawAtPoint:NSMakePoint(NSMidX(liveBadge) - liveTextSize.width / 2.0,
                                              NSMidY(liveBadge) - liveTextSize.height / 2.0)
                    withAttributes:liveAttrs];
            [NSGraphicsContext restoreGraphicsState];
            nameEndX = NSMaxX(liveBadge);
        }
...
@property (nonatomic, strong) NSWindow *debugWindow;
@property (nonatomic, strong) NSTextView *debugTextView;
@property (nonatomic, strong) NSTimer *debugRefreshTimer;
@property (nonatomic, copy) NSString *lastDebugWindowExplorerText;
...
@property (nonatomic, assign) NSWindow *hostWindowForMovementTracking;
@end
...
- (instancetype)init {
    ...
    self.debugMode = DO_DEFAULT_DEBUG_MODE;
    ...
    if (self.debugMode) [self createDebugWindow];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hostWindowWillMove:)
                                                 name:NSWindowWillMoveNotification
                                               object:nil];
    [self selectHostWindowForMovementTracking];
    [self attachOverlayWindowsToHostWindow];
...
- (void)dealloc {
    ...
    [self.debugRefreshTimer invalidate];
...
- (void)loadConfiguration {
    ...
    if ([config[@"debug_mode"] respondsToSelector:@selector(boolValue)]) {
        self.debugMode = [config[@"debug_mode"] boolValue];
    }
...
- (BOOL)isHostAppFrontmost {
    ...
    // Java-based games can have a frontmost bundle identifier that differs from
    // the injected process bundle. The process ID is the authoritative match.
    if (frontmost.processIdentifier == getpid()) return YES;
...
- (void)createDebugWindow {
    NSScreen *screen = NSScreen.mainScreen;
    if (!screen || self.debugWindow) return;

    NSRect visible = screen.visibleFrame;
    NSRect frame = NSMakeRect(NSMidX(visible) - 310.0, NSMidY(visible) - 210.0, 620.0, 420.0);
    NSWindowStyleMask style = NSWindowStyleMaskTitled |
        NSWindowStyleMaskClosable |
        NSWindowStyleMaskMiniaturizable |
        NSWindowStyleMaskResizable;
    self.debugWindow = [[NSWindow alloc] initWithContentRect:frame
                                                    styleMask:style
                                                      backing:NSBackingStoreBuffered
                                                        defer:NO];
    self.debugWindow.title = @"Discord Overlay Window Explorer";
    self.debugWindow.minSize = NSMakeSize(420.0, 260.0);
    self.debugWindow.releasedWhenClosed = NO;
    self.debugWindow.collectionBehavior = NSWindowCollectionBehaviorCanJoinAllSpaces |
        NSWindowCollectionBehaviorFullScreenAuxiliary;

    NSScrollView *scrollView = [[NSScrollView alloc] initWithFrame:self.debugWindow.contentView.bounds];
    scrollView.autoresizingMask = NSViewWidthSizable | NSViewHeightSizable;
    scrollView.hasVerticalScroller = YES;
    scrollView.hasHorizontalScroller = YES;
    scrollView.borderType = NSBezelBorder;

    self.debugTextView = [[NSTextView alloc] initWithFrame:scrollView.bounds];
    self.debugTextView.editable = NO;
    self.debugTextView.selectable = YES;
    self.debugTextView.richText = NO;
    self.debugTextView.automaticQuoteSubstitutionEnabled = NO;
    self.debugTextView.font = [NSFont monospacedSystemFontOfSize:12.0 weight:NSFontWeightRegular];
    self.debugTextView.textColor = NSColor.labelColor;
    self.debugTextView.backgroundColor = NSColor.textBackgroundColor;
    self.debugTextView.textContainerInset = NSMakeSize(10.0, 10.0);
    scrollView.documentView = self.debugTextView;
    self.debugWindow.contentView = scrollView;

    [self refreshDebugWindowExplorer];
    self.debugRefreshTimer = [NSTimer scheduledTimerWithTimeInterval:0.5
                                                                target:self
                                                              selector:@selector(refreshDebugWindowExplorer)
                                                              userInfo:nil
                                                               repeats:YES];
    [[NSRunLoop mainRunLoop] addTimer:self.debugRefreshTimer forMode:NSRunLoopCommonModes];
    [self.debugWindow makeKeyAndOrderFront:nil];
}

- (void)refreshDebugWindowExplorer {
    if (!self.debugMode || !self.debugTextView) return;
    if (self.debugWindow.isKeyWindow && self.debugTextView.selectedRange.length > 0) return;

    NSArray<NSWindow *> *windows = [NSApp.windows sortedArrayUsingComparator:^NSComparisonResult(NSWindow *left, NSWindow *right) {
        return [NSStringFromClass(left.class) compare:NSStringFromClass(right.class)];
    }];
    NSRunningApplication *frontmost = NSWorkspace.sharedWorkspace.frontmostApplication;
    NSString *frontmostName = frontmost.localizedName ?: @"Unknown";
    NSString *frontmostBundleId = frontmost.bundleIdentifier ?: @"—";
    NSString *hostBundleId = self.hostBundleIdentifier.length > 0 ? self.hostBundleIdentifier : @"—";
    BOOL hostFrontmost = [self isHostAppFrontmost];
    NSUInteger voiceUserCount = self.voiceView.users.count;
    NSWindow *trackedHost = self.hostWindowForMovementTracking;
    NSString *trackedHostName = trackedHost.title.length > 0 ? trackedHost.title : @"Untitled";
    NSUInteger attachedOverlayCount = 0;
    for (NSWindow *window in @[self.voiceWindow ?: NSNull.null, self.controlsWindow ?: NSNull.null,
                                self.notificationsWindow ?: NSNull.null, self.messageWindow ?: NSNull.null,
                                self.hudWindow ?: NSNull.null, self.hotkeyConfigWindow ?: NSNull.null]) {
        if ((id)window != NSNull.null && window.parentWindow == trackedHost) attachedOverlayCount++;
    }
    NSMutableString *text = [NSMutableString stringWithFormat:@"NSWindow Explorer  •  %lu windows\nHost: pid %d, bundle %@\nFrontmost: %@ (pid %d, bundle %@)\nTracked host: %@ (%@), attached overlays: %lu\nOverlay state: host-frontmost=%@, voice-users=%lu, voice-enabled=%@\n\n",
                             (unsigned long)windows.count,
                             getpid(), hostBundleId, frontmostName, frontmost.processIdentifier, frontmostBundleId,
                             trackedHost ? trackedHostName : @"None", trackedHost ? NSStringFromClass(trackedHost.class) : @"—",
                             (unsigned long)attachedOverlayCount,
                             hostFrontmost ? @"yes" : @"no", (unsigned long)voiceUserCount,
                             self.voiceVisible ? @"yes" : @"no"];
    NSUInteger index = 1;
    for (NSWindow *window in windows) {
        NSString *name = window.title.length > 0 ? window.title : @"Untitled";
        NSString *identifier = window.identifier.length > 0 ? window.identifier : @"—";
        NSString *parentName = window.parentWindow.title.length > 0 ? window.parentWindow.title :
            (window.parentWindow ? @"Untitled" : @"None");
        NSRect frame = window.frame;
        [text appendFormat:@"%02lu  %@\n    class: %@  id: %@  parent: %@\n    frame: {%.0f, %.0f, %.0f, %.0f}  visible: %@  key: %@\n\n",
         (unsigned long)index++, name, NSStringFromClass(window.class), identifier, parentName,
         frame.origin.x, frame.origin.y, frame.size.width, frame.size.height,
         window.isVisible ? @"yes" : @"no", window.isKeyWindow ? @"yes" : @"no"];
    }
    if ([self.lastDebugWindowExplorerText isEqualToString:text]) return;
    self.lastDebugWindowExplorerText = text;
...
- (void)createVoiceWindow {
    ...
    self.voiceWindow = [[NSWindow alloc] initWithContentRect:frame
                                                   styleMask:NSWindowStyleMaskBorderless
                                                     backing:NSBackingStoreBuffered
                                                       defer:NO];
    self.voiceWindow.title = @"Discord Overlay Voice";
...
- (void)createControlsWindow {
    ...
    self.controlsWindow = [[NSWindow alloc] initWithContentRect:frame
                                                      styleMask:NSWindowStyleMaskBorderless
                                                        backing:NSBackingStoreBuffered
                                                          defer:NO];
    self.controlsWindow.title = @"Discord Overlay Controls";
...
- (void)createNotificationsWindow {
    ...
    self.notificationsWindow = [[NSWindow alloc] initWithContentRect:frame
                                                           styleMask:NSWindowStyleMaskBorderless
                                                             backing:NSBackingStoreBuffered
                                                               defer:NO];
    self.notificationsWindow.title = @"Discord Overlay Notifications";
...
- (void)createMessageWindow {
    ...
    self.messageWindow = [[DOOverlayChatWindow alloc] initWithContentRect:frame
                                                      styleMask:NSWindowStyleMaskBorderless
                                                        backing:NSBackingStoreBuffered
                                                          defer:NO];
    self.messageWindow.title = @"Discord Overlay Chat";
...
- (void)createHUDWindow {
    ...
    self.hudWindow = [[NSWindow alloc] initWithContentRect:frame
                                                 styleMask:NSWindowStyleMaskBorderless
                                                   backing:NSBackingStoreBuffered
                                                   defer:NO];
    self.hudWindow.title = @"Discord Overlay Edit Mode";
...
- (BOOL)isOverlayOrExplorerWindow:(NSWindow *)window {
    return window == self.voiceWindow ||
        window == self.controlsWindow ||
        window == self.notificationsWindow ||
        window == self.messageWindow ||
        window == self.hudWindow ||
        window == self.hotkeyConfigWindow ||
        window == self.debugWindow;
}

- (void)selectHostWindowForMovementTracking {
    NSWindow *bestWindow = nil;
    CGFloat bestArea = 0.0;
    for (NSWindow *window in NSApp.windows) {
        if ([self isOverlayOrExplorerWindow:window] || !window.isVisible) continue;
        NSRect frame = window.frame;
    ...
    self.hostWindowForMovementTracking = bestWindow;
}

- (void)attachOverlayWindow:(NSWindow *)window toHostWindow:(NSWindow *)hostWindow {
    if (!window || !hostWindow || window == self.debugWindow || window == hostWindow) return;
    if (window.parentWindow == hostWindow) return;
    if (window.parentWindow) [window.parentWindow removeChildWindow:window];
    [hostWindow addChildWindow:window ordered:NSWindowAbove];
}

- (void)attachOverlayWindowsToHostWindow {
    NSWindow *hostWindow = self.hostWindowForMovementTracking;
    if (!hostWindow) return;
    [self attachOverlayWindow:self.voiceWindow toHostWindow:hostWindow];
    [self attachOverlayWindow:self.controlsWindow toHostWindow:hostWindow];
    [self attachOverlayWindow:self.notificationsWindow toHostWindow:hostWindow];
    [self attachOverlayWindow:self.messageWindow toHostWindow:hostWindow];
    [self attachOverlayWindow:self.hudWindow toHostWindow:hostWindow];
    [self attachOverlayWindow:self.hotkeyConfigWindow toHostWindow:hostWindow];
}

- (void)hostWindowWillMove:(NSNotification *)note {
    NSWindow *window = note.object;
    if (![window isKindOfClass:[NSWindow class]] || [self isOverlayOrExplorerWindow:window]) return;
    if (!self.hostWindowForMovementTracking) {
        self.hostWindowForMovementTracking = window;
        [self attachOverlayWindowsToHostWindow];
    }
}
...
- (void)updateNotificationsWindowVisibility {
    ...
    if (![self isHostAppFrontmost]) {
        [self hideOverlayWindow:self.notificationsWindow];
        return;
    }
    BOOL shouldShow = self.editMode || (self.notificationsVisible && hasNotifs);
